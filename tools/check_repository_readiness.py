...
_DEFAULT_SPEC = Path("validation/repository_readiness.json")
...
def validate_repository_readiness(
    ...
    failures.extend(_validate_coverage_gate(root))
...
def _validate_coverage_gate(root: Path) -> list[str]:
    pyproject_path = root / "pyproject.toml"
    with pyproject_path.open("rb") as handle:
        pyproject = tomllib.load(handle)
    pytest_options = pyproject.get("tool", {}).get("pytest", {}).get("ini_options", {})
    coverage_report = pyproject.get("tool", {}).get("coverage", {}).get("report", {})
    failures: list[str] = []
    addopts = pytest_options.get("addopts", "")
    if not isinstance(addopts, str) or "--cov-fail-under=100" not in addopts:
        failures.append("pytest coverage gate must be --cov-fail-under=100")
    if coverage_report.get("fail_under") != 100:
        failures.append("coverage report fail_under must be 100")
    return failures
